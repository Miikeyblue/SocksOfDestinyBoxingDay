...
function showOverlay(title, sub, text, mode) {
  ...
  endTurnBtn.style.display = (mode === "turn") ? "none" : (mode === "roundEnd" ? "none" : "block");

  rollBtn.style.display = (mode === "turn") ? "block" : "none";
  if (mode === "roundEnd") { rollBtn.style.display = "none"; }
...
function showRoundEnd() {
  ...
  roundEndConfirm.onclick = () => {
    ...
    if (!staying.length) {
      saveState();
      typeText(overlayText, "Everyone chose to leave. Game over. Crack open the socks and reveal them. 🎁");
      endTurnBtn.style.display = "block";
      pickTurnBtn.disabled = true;
      resetTurnsBtn.disabled = false;
      renderTurnRemaining();
